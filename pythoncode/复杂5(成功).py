...
class UC_Model:
    ...
    def create_model(self):
        ...
        def power_balance_rule(model, t):
            ...
            for load in model.Loads:
                # 获取负荷系数
                load_row = self.loads[self.loads['负荷编号'] == load]
                if not load_row.empty:
                    coef = load_row['有功比例系数'].values[0]
                else:
                    coef = 0
                ...
                # 修正：确保curve_val和coef都为float，避免字符串与float相乘
                try:
                    coef = float(coef)
                except Exception:
                    coef = 0.0
                ...
                total_load += coef * curve_val - model.load_shed[load, t]
            
            return (thermal_gen + wind_gen + solar_gen + hydro_gen + ess_discharge == 
                    total_load + ess_charge)
