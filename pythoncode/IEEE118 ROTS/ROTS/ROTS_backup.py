...
class UC_Model:
    ...
    def create_model(self):
        ...
        def total_cost_rule(model):
            ...
            renewable_incentive = -100  # 调整到合理的100元/MWh激励，避免成本过高
        ...
        def solar_min_output_rule(model, unit, t):
            ...
            if capacity > 1:
                ...
                # 白天时段强制光伏出力，确保发电量随时间变化
                if 6 <= hour <= 18:  # 白天时段
                    if 10 <= hour <= 14:  # 强光照时段
                        min_factor = 0.6  # 至少60%容量
                    elif 8 <= hour <= 16:  # 中等光照
                        min_factor = 0.4  # 至少40%容量
                    else:  # 弱光照
                        min_factor = 0.2  # 至少20%容量
        ...
        # 6.5 储能充放电互斥约束（放宽限制，提高储能利用率）
        def ess_charge_discharge_exclusive_rule(model, unit, t):
            max_charge = self.ess_dict[unit].get('MaxChargePower', 0)
            max_discharge = self.ess_dict[unit].get('MaxDisPower', 0)
            
            # 放宽约束：允许同时充放电，但总功率不超过最大值的80%
            max_power = max(max_charge, max_discharge)
            if max_power > 0:
                return model.ess_charge[unit, t] + model.ess_discharge[unit, t] <= max_power * 0.8
            else:
                return pyo.Constraint.Skip
        
        self.model.ess_exclusive_con = pyo.Constraint(
            self.model.ESSUntis, self.model.T, rule=ess_charge_discharge_exclusive_rule
        )
        
        # 移除二进制约束以保持线性规划

        # 7. 直流潮流约束
        logging.info("启用直流潮流约束...")
    ...
    def plot_generation_mix(self, output_path):
        ...
        # 弃电量显示（淡色堆叠在发电上方，表示未利用的潜力）
        curtail_bottom = bottom
        
        # 弃风（淡绿色，半透明）
        if max(wind_curtail) > 0.1:
            p7 = ax.bar(hours, wind_curtail, width, bottom=curtail_bottom, 
                       label='Wind_Curtailment', color='#44ff44', alpha=0.3, 
                       hatch='///', edgecolor='green', linewidth=0.5)
            curtail_bottom += wind_curtail
        
        # 弃光（淡黄色，半透明）
        if max(solar_curtail) > 0.1:
            p8 = ax.bar(hours, solar_curtail, width, bottom=curtail_bottom, 
                       label='Solar_Curtailment', color='#ffff44', alpha=0.3,
                       hatch='///', edgecolor='orange', linewidth=0.5)
            curtail_bottom += solar_curtail
        
        # 弃水（淡蓝色，半透明）
        if max(hydro_curtail) > 0.1:
            p9 = ax.bar(hours, hydro_curtail, width, bottom=curtail_bottom, 
                       label='Hydro_Curtailment', color='#4444ff', alpha=0.3,
                       hatch='///', edgecolor='blue', linewidth=0.5)
        
        # 储能充电（更突出的颜色，显示在负轴）
        ess_charge_negative = [-x for x in ess_charge]  # 转为负值
        p6 = ax.bar(hours, ess_charge_negative, width, label='Power_StoreCharge', 
                   color='#666666', alpha=0.9, edgecolor='black', linewidth=1)
        
        # 添加储能净出力的连线图（更突出储能的动态特性）
        ess_net = ess_discharge - ess_charge
        ax.plot(hours, ess_net, 'o-', color='purple', linewidth=3, markersize=4,
               label='ESS_Net_Power', alpha=0.8)
        
        # 计算净发电量（与功率平衡约束一致）
        net_generation = thermal_gen + wind_gen + solar_gen + hydro_gen + ess_discharge
        
        # 只添加两条关键线：总负荷曲线和备用要求线
        total_load = self.load_curve[:self.T] if len(self.load_curve) >= self.T else [0] * self.T
        ax.plot(hours, total_load, 'k--', linewidth=3, label='Total Demand', alpha=0.9)
        
        # 添加备用容量要求线
        reserve_req = np.array(total_load[:self.T]) * 0.20  # 20%备用要求
        ax.plot(hours, np.array(total_load[:self.T]) + reserve_req, 'g:', linewidth=3, 
               label='Reserve Requirement (20%)', alpha=0.9)
        ...
        if max(total_load_shed) > 0.1:
            ax.plot(hours, effective_load, 'm--', linewidth=2, label='Effective Load (after shedding)', alpha=0.8)
        ...
        # 计算实际应该满足的净发电量（根据功率平衡约束的负荷侧）
        # 应该等于：total_load - total_shed + ess_charge
        theoretical_net_generation = []
        for t in range(self.T):
            # 基础负荷
            ...
            for unit in self.model.ESSUntis:
                charge += safe_value(self.model.ess_charge[unit, t])
            theoretical_net_generation.append(base_load - shed + charge)
        ...
        for t in range(self.T):
            # 火电机组总容量（开机状态的）
            thermal_capacity = sum(safe_value(self.model.thermal_status[unit, t]) * 
                                 self.thermal_dict[unit].get('Capacity', 0) 
                                 for unit in self.model.ThermalUnits)
            
            # 可再生能源按当前出力计算（已经是最大可能）
            wind_capacity = wind_gen[t]
            solar_capacity = solar_gen[t] 
            hydro_capacity = hydro_gen[t]
            
            # 储能最大放电容量
            ess_capacity = sum(self.ess_dict[unit].get('MaxDisPower', 0) 
                             for unit in self.model.ESSUntis)
            
            total_capacity = thermal_capacity + wind_capacity + solar_capacity + hydro_capacity + ess_capacity
            total_available_capacity.append(total_capacity)
        
        # 计算实际备用（总可用容量减去实际需求）
        actual_load_with_shed = []
        for t in range(self.T):
            effective_load = total_load[t] if t < len(total_load) else 0
            # 减去切负荷
            for load in self.model.Loads:
                effective_load -= safe_value(self.model.load_shed[load, t])
            # 加上储能充电需求
            effective_load += sum(safe_value(self.model.ess_charge[unit, t]) for unit in self.model.ESSUntis)
            actual_load_with_shed.append(effective_load)
        
        available_reserve = np.array(total_available_capacity) - np.array(actual_load_with_shed)
        ...
        info_text = (f'最大需求: {max_demand:.1f} MW\n'
                    f'最小备用: {min_reserve:.1f} MW\n'
        ...
        all_positive_values.extend(total_available_capacity)  # 总可用容量
        
        # 收集所有负值数据
        all_negative_values.extend(ess_charge_negative)  # 储能充电（负值）
