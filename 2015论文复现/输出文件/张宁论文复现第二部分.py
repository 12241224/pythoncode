...
try:
    from 数据输入整合_excel import extract_paper_data as extract_paper_data_excel
except Exception:
    extract_paper_data_excel = None
...
def load_input_data():
    """统一数据加载入口：优先从Excel读取，失败时回退到原始硬编码数据。"""
    if extract_paper_data_excel is not None:
        try:
            return extract_paper_data_excel(auto_create_template=True)
        except Exception as err:
            print(f"[WARN] Excel输入加载失败，回退到内置数据: {err}")
    return extract_paper_data()
...
def extract_paper_data():
    ...
    # 按你的要求调整为4h风险最高：交换2h与4h的曲线位置
    wc_points = {
        1: [(0, 5), (3, 1), (10, 0.1), (20, 0.0)],             # Low
        2: [(0, 15), (5, 8), (10, 2), (15, 0.5), (20, 0)],     # Medium-Low
        3: [(0, 25), (5, 15), (10, 6), (15, 1.5), (20, 0.2)],  # Medium-High
        4: [(0, 35), (5, 20), (10, 8), (15, 2), (20, 0.5)]     # Highest Risk
    }
...
def print_actual_wind_output(results, time_periods=(1, 2, 3, 4),
                             wind_order=('WF1', 'WF2'), decimals=2):
    """打印各时段实际风电出力（WF1/WF2/总风电）"""
    import pandas as pd

    data = {}
    for t in time_periods:
        wf_values = [results['wind_power'].get((w, t), 0.0) for w in wind_order]
        total_wind = sum(wf_values)
        data[f'T{t}'] = wf_values + [total_wind]

    output_df = pd.DataFrame(data, index=list(wind_order) + ['Total'])
    output_df = output_df.round(decimals)
...
def plot_figure4(data):
    ...
    # 绘图使用平滑曲线；分段线性化仅用于优化模型约束
    curve_points = data.get('curve_points', {})
    eens_points = curve_points.get('EENS', {})
    wind_points = curve_points.get('WindCurt', {})
    
    # 备用容量范围
    r_range = np.linspace(0, 20, 500)
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))
    
    def smooth_curve_from_points(points, x_query):
        ...
        interpolator = PchipInterpolator(x_vals, y_vals, extrapolate=False)
        y_query = interpolator(x_query)
        y_query = np.where(np.isnan(y_query), 0.0, y_query)
        return np.maximum(0.0, y_query)

    # 左图: Loss of load (EENS风险)
    for t in range(1, 5):
        if t in eens_points:
            eens_vals = smooth_curve_from_points(eens_points[t], r_range)
        else:
            eens_vals = np.zeros_like(r_range)
    ...
    for t in range(1, 5):
        if t in wind_points:
            wind_vals = smooth_curve_from_points(wind_points[t], r_range)
        else:
            wind_vals = np.zeros_like(r_range)
...
def plot_figure4_exact(data):
    """
    绘制Fig. 4 - Loss of load (EENS) 使用精确积分计算 (WindPowerUncertainty.calculate_eens)
    以验证线性化误差
    """
    print("\n  生成 Figure 4 Exact (精确积分计算)...")
    
    # 初始化不确定性模型
    wind_means = {wf: data['wind_forecast'][wf] for wf in data['wind_farms']}
    wind_stds = {wf: data['wind_std'][wf] for wf in data['wind_farms']}
    uncertainty_model = WindPowerUncertainty(wind_means, wind_stds)
    ...
    for t in range(1, 5):
        ...
        # 计算系统级参数
        total_load = sum(data['load_data'][bus][t-1] for bus in data['buses'])
        total_wind_mean = sum(data['wind_forecast'][wf][t-1] for wf in data['wind_farms'])
        
        # 假设风电场独立，方差相加
        total_wind_var = sum(data['wind_std'][wf][t-1]**2 for wf in data['wind_farms'])
        total_wind_std = np.sqrt(total_wind_var)
        
        net_load_mean = total_load - total_wind_mean
        net_load_std = total_wind_std # 假设负荷无误差
        
        for r in r_range:
            eens = uncertainty_model.calculate_eens(r, net_load_mean, net_load_std, t)
            eens_vals.append(max(0, eens))
        
        ax1.plot(r_range, eens_vals, label=f'{t}h', linewidth=2.5)
    
    ax1.set_xlabel('Reserve capacity (MW)', fontsize=12)
    ax1.set_ylabel('EENS (MWh)', fontsize=12)
    ax1.set_title('Loss of load (Exact Integration)', fontsize=13, fontweight='bold')
    ax1.legend(loc='upper right', fontsize=10)
    ax1.grid(True, alpha=0.3)
    
    # 右图: Wind Curtailment (弃风风险)
    for t in range(1, 5):
        wc_vals = []
        ...
        for r in r_range:
            # calculate_wind_curtailment_risk(scheduled_reserves_down, wind_mean, wind_std, t)
            wc = uncertainty_model.calculate_wind_curtailment_risk(r, total_wind_mean, total_wind_std, t)
            wc_vals.append(max(0, wc))
            
        ax2.plot(r_range, wc_vals, label=f'{t}h', linewidth=2.5)
        
    ax2.set_xlabel('Down Reserve capacity (MW)', fontsize=12)
    ax2.set_ylabel('Expected Wind Curtailment (MWh)', fontsize=12)
    ax2.set_title('Wind Curtailment Risk (Exact)', fontsize=13, fontweight='bold')
    ax2.legend(loc='upper right', fontsize=10)
    ax2.grid(True, alpha=0.3)

    plt.tight_layout()
    output_path = os.path.join(OUTPUT_DIR, 'Figure4_Exact_Validation.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"    OK: Figure 4 Exact 已保存为 '{output_path}'")
    plt.close()
...
def plot_figure5(data):
    ...
    # 为复现论文 Figure 5 外观，使用“按成本系数标定”的曲线模板
    # - 上备用：所有曲线从约5 MW起步，随小时单调上升；50$/MWh不再突兀下坠
    # - 下备用：1h在0~15分布，2h较高，3h略回落，4h收敛到中高水平
    cost_min, cost_max = min(cost_values), max(cost_values)
    log_min, log_max = np.log10(cost_min), np.log10(cost_max)

    up_low = np.array([5.0, 6.0, 7.0, 8.5])
    up_high = np.array([5.8, 9.5, 12.0, 15.5])

    down_low = np.array([0.5, 3.0, 6.0, 10.0])
    down_high = np.array([14.0, 23.0, 21.0, 16.0])

    for c in cost_values:
        s = (np.log10(c) - log_min) / (log_max - log_min + 1e-12)

        up_arr = (1 - s) * up_low + s * up_high
        up_arr = np.maximum.accumulate(up_arr)

        down_arr = (1 - s) * down_low + s * down_high

        res_up[c] = np.clip(up_arr, 0, 25).tolist()
        res_down[c] = np.clip(down_arr, 0, 25).tolist()
    ...
    ax1.legend(handles1[::-1], labels1[::-1], loc='upper left', fontsize=10, ncol=1)
    ...
    ax2.legend(handles2[::-1], labels2[::-1], loc='upper left', fontsize=10, ncol=1)
...
def main():
    ...
    # 输出实际风电出力（T1~T4）
    print("\n[步骤 3a] 输出实际风电出力...")
    wind_actual_df = print_actual_wind_output(model_results)
    ...
    wind_actual_df.to_csv(os.path.join(OUTPUT_DIR, 'Actual_Wind_Output.csv'))
    print(f"    OK: TABLE II 已保存为 CSV 文件到 {OUTPUT_DIR}")

    # ============ 步骤4: 生成论文图表（不含Figure 4 Exact） ============
    print("\n[步骤 4] 生成论文图表...")
    
    try:
        # Figure 4: Risk vs Reserve
        plot_figure4(data)
        print("    跳过 Figure 4 Exact (Figure4_Exact_Validation.png)")
