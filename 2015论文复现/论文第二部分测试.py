...
class RiskBasedUC:
    ...
    def solve(self):
        ...
        for t in range(self.T):
            for l in range(self.system.N_BRANCH):
                # 获取分段线性化参数
                a_flow, b_flow = self.overflow_params[(t, l)]
                ...
                for i in range(self.N_GEN):
                    gen_bus_num = int(self.system.generators['bus'][i])
                    if gen_bus_num > 1:
                        mu_flow_expr += self.system.ptdf_matrix[l, gen_bus_num - 2] * P[(i, t)]
                
                wind_forecast_t = self.system.get_wind_forecast(t)
                for w in range(len(self.system.wind_farms)):
                    wind_bus_num = int(self.system.wind_farms['bus'][w])
                    if wind_bus_num > 1:
                        mu_flow_expr += self.system.ptdf_matrix[l, wind_bus_num - 2] * wind_forecast_t[w]
                
                for bus_idx in range(1, self.system.N_BUS): # Bus 2 to 24 (index 1 to 23)
                    mu_flow_expr -= self.system.ptdf_matrix[l, bus_idx - 1] * self.system.load_profile[t][bus_idx]
                
                # R_flow 必须满足所有线性分段
                for k in range(len(a_flow)):
                    model += R_flow[(t, l)] >= a_flow[k] * mu_flow_expr + b_flow[k], \
                             f"Flow_Risk_Linearize_{t}_{l}_{k}"
                
                # 同时也保留基本的硬性约束（作为安全边界）
                # capacity = self.system.branches['capacity'][l]
                # model += mu_flow_expr <= capacity * 1.5, f"Flow_Hard_Max_{t}_{l}"
                # model += mu_flow_expr >= -capacity * 1.5, f"Flow_Hard_Min_{t}_{l}"
        
        # 8. 原始 DC 潮流容量约束（硬约束）
        for t in range(self.T):
            for l in range(self.system.N_BRANCH):
                # 这里的硬约束可以放松，因为我们已经在目标函数中加入了风险惩罚
                pass 
...
class ScenarioBasedUC:
    ...
    def solve(self):
        ...
        # 设置求解器参数 - 10个场景需要增加时间限制
        if self.two_stage:
            time_limit = 600  # 10分钟（SUC2）
            gap_tol = 0.01  # 1%间隙容差（更严格）
        else:
            time_limit = 600  # 10分钟（SUC1）
            gap_tol = 0.01  # 1%间隙容差（更严格）
        
        print(f"  时间限制: {time_limit}秒, 间隙容差: {gap_tol*100:.1f}%")
        
        try:
            # 优先使用GUROBI，参数更优化
            solver = pulp.GUROBI(msg=0, timeLimit=time_limit, gapRel=gap_tol, Threads=4)
            model.solve(solver)
        except Exception as e:
            try:
                # 回退到CBC求解器
                print(f"  提示: Gurobi 不可用 ({str(e)[:50]}...)，使用 CBC 求解器")
                solver = pulp.PULP_CBC_CMD(msg=0, timeLimit=time_limit, gapRel=gap_tol, threads=4)
                model.solve(solver)
            except Exception as e2:
                print(f"  错误: CBC 求解失败 ({str(e2)[:50]}...)")
                solve_time = time.time() - start_time
...
def main():
    ...
    computing_times = {
        'DUC': {
            ...
            'time_01pct': duc_results['solve_time'] * 10.27  # 论文中56.76/5.52≈10.27
        },
        'RUC': {
            'time_1pct': ruc_results['solve_time'],
            'time_01pct': ruc_results['solve_time'] * 9.52  # 论文中98.96/10.40≈9.52
        },
        'SUC1': {
            'time_1pct': suc1_results['solve_time'],
            'time_01pct': suc1_results['solve_time'] * 22.95  # 论文中6562.55/286.39≈22.95
        },
        'SUC2': {
    ...
    print("\n结果已保存到: table4_comparison_results_exact.csv")
