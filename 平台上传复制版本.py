...
if __name__ == '__main__':
    ...
    metrics = predictor.fit(df, n_estimators=50)

    prediction = predictor.predict(df)

    print("训练集R2:", metrics.get('train_score'))
    print("测试集R2:", metrics.get('test_score'))
    print("预测结果(前10个):", prediction[:10])

    # 保存预测结果
    out_path = Path("predicted_prices.csv")
    pd.DataFrame({"predicted_price": prediction}).to_csv(out_path, index=False)
    print("已保存预测结果:", out_path.resolve())

    # 可视化（若环境安装了 matplotlib）
    try:
        import matplotlib.pyplot as plt

        plt.figure(figsize=(10, 4))
        plt.plot(prediction, label='Predicted Price')
        plt.title('Forecasted Electricity Price')
