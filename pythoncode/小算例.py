...
model.p_100mw_sq = pyo.Var(model.G, model.T, within=pyo.NonNegativeReals)  # 机组出力平方（100MW^2）

# 定义分段点
num_segments = 10
segment_points = np.linspace(0, 1.6, num_segments)  # 根据 Pmax/Pmin 的范围调整

# 引入辅助变量
model.lambda_ = pyo.Var(model.G, model.T, range(num_segments), within=pyo.Binary)  # 分段权重
model.p_100mw_linear = pyo.Var(model.G, model.T, within=pyo.NonNegativeReals)  # 线性化后的 p_100mw
...
def objective_rule(model):
    ...
    for g in model.G:
        for t in model.T:
            # 运行成本（将出力转换为100MW计算）
            total_cost += (generators[g]['a'] * model.p_100mw_sq[g, t] + generators[g]['b'] * model.p_100mw[g, t] + generators[g]['c'])
            # 启停成本
...
# 2. 线性化约束
def linearize_p_100mw_rule(model, g, t):
    return model.p_100mw[g, t] == model.p[g, t] / 100

model.linearize_p_100mw = pyo.Constraint(model.G, model.T, rule=linearize_p_100mw_rule)

# 分段权重约束
def lambda_sum_rule(model, g, t):
    return sum(model.lambda_[g, t, s] for s in range(num_segments)) == 1

model.lambda_sum = pyo.Constraint(model.G, model.T, rule=lambda_sum_rule)

# 线性化后的 p_100mw 约束
def p_100mw_linear_rule(model, g, t):
    return model.p_100mw_linear[g, t] == sum(model.lambda_[g, t, s] * segment_points[s] for s in range(num_segments))

model.p_100mw_linear_constr = pyo.Constraint(model.G, model.T, rule=p_100mw_linear_rule)

# 线性化后的 p_100mw_sq 约束
def p_100mw_sq_linear_rule(model, g, t):
    return model.p_100mw_sq[g, t] == sum(model.lambda_[g, t, s] * (segment_points[s]**2) for s in range(num_segments))

model.p_100mw_sq_linear_constr = pyo.Constraint(model.G, model.T, rule=p_100mw_sq_linear_rule)
...
for g in model.G:
    print(f"\n机组 {g}:")
    for t in model.T:
        status = "运行" if model.u[g, t].value > 0.5 else "停机"
        print(f"  时段 {t:2d}: 状态={status}, 出力={model.p[g, t].value:.2f} MW")
